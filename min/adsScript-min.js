...
changeAd();setInterval(()=>{changeAd()},5000)}}
function adsMultipleAtATime(elem,arr,max){if(arr.length>=1&&arr.length<=max){arr.forEach((ad)=>{const divAd=document.createElement('div');divAd.classList.add('ad');divAd.id=ad.id;const linkAd=document.createElement('a');linkAd.href=ad.link;linkAd.target='_blank';const imgAd=document.createElement('img');imgAd.src=ad.photoURL;imgAd.alt=`Banner do anúncio ${ad.name}`;linkAd.appendChild(imgAd);divAd.appendChild(linkAd);elem.appendChild(divAd)})}else if((arr.length>1)&(arr.length>max)){refreshAdsMultiple(elem,arr,max);setInterval(()=>refreshAdsMultiple(elem,arr,max),5000)}}
function refreshAdsMultiple(elem,arr,max){const random=generateRandomArray(0,arr.length-1,max);elem.querySelectorAll('.ad').forEach((ad)=>ad.remove());random.forEach((num)=>{const ad=arr[num];const divAd=document.createElement('div');divAd.classList.add('ad');divAd.id=ad.id;const linkAd=document.createElement('a');linkAd.href=ad.link;linkAd.target='_blank';const imgAd=document.createElement('img');imgAd.src=ad.photoURL;imgAd.alt=`Banner do anúncio ${ad.name}`;linkAd.appendChild(imgAd);divAd.appendChild(linkAd);elem.appendChild(divAd)})}
function generateRandomArray(min,max,size,jump=1){const result=new Set();while(result.size<size){const num=Math.floor(Math.random()*(max-min+1))+min;let isValid=!0;for(const existingNum of result)
if(Math.abs(num-existingNum)<jump){isValid=!1;break}
if(isValid)result.add(num)}
return Array.from(result)}
function initRenderAds(){const divMainAds='<div class="main-ads ads-region"></div>';if(!isPostPage()){const posts=document.querySelectorAll('.post-outer-container');for(let i=0;i<posts.length;i++)
if((i+2)%3===0)
